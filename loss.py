...
__all__ = ["Filter_MSE_Loss", "FilterMSELoss", "MSELoss", "HuberLoss", "MAELoss", "SmoothMSELoss"]


class Filter_MSE_Loss(nn.Layer):
    def __init__(self, **kwargs):
        super(Filter_MSE_Loss, self).__init__()

    def forward(self, pred, target):
        # Remove bad target
        mask = (target != -99999)
        diff = pred - target
        diff = paddle.multiply(diff, paddle.to_tensor(mask).astype('float32'))
        loss = paddle.mean(paddle.square(diff))
        return loss
