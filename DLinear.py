...
class WPFModel(nn.Layer):
    ...
    def __init__(self, settings):
        ...
        self.to_channels = nn.Linear(self.enc_in, self.out_channels)
        # season
        self.Linear_Seasonal = nn.Linear(self.input_len, self.output_len + self.label_len)
        x1 = a * paddle.ones([self.input_len, self.output_len + self.label_len], dtype='float32')
        self.Linear_Seasonal.weight = paddle.create_parameter(x1.shape, dtype='float32',
                                                              default_initializer=nn.initializer.Assign(x1))
        # trend
        self.Linear_Trend = nn.Linear(self.input_len, self.output_len + self.label_len)
        y1 = a * paddle.ones([self.input_len, self.output_len + self.label_len], dtype='float32')
    ...
    def forward(self, batch_x):
        ...
        seasonal_init = self.to_channels(seasonal_init)
        trend_init = self.to_channels(trend_init)
