...
class WPFDataset(Dataset):
    ...
    def __read_data__(self):
        df_raw = pd.read_csv(os.path.join(self.data_path, self.filename), parse_dates=['DATATIME'],
                             infer_datetime_format=True, dayfirst=True,
                             dtype={'WINDDIRECTION': np.float64, 'HUMIDITY': np.float64, 'PRESSURE': np.float64})
        # TODO 注意这个时间数据去重
        # df_raw.drop_duplicates(subset=['DATATIME'], keep='first', inplace=True)
    ...
    def __get_data_x_y(self):
        ...
        for i in range(134):  # 134 = capacity
            str_i = str(i)
            col_ls.append(str_i)
    ...
    def __get_data__(self):
        data_x, data_y = self.__get_data_x_y()
        return data_x, data_y
    ...
    def data_preprocess(self, df_data):
        feature_name = [
            ...
            if 'ROUND(A.WS,1)' not in n and 'ROUND(A.POWER,0)' not in n and
               'YD15' not in n
        ]
        ...
        new_df_data = new_df_data.replace(
            to_replace=np.nan, value=-99999, inplace=False)
        # negative->0.0
...
# TODO 更改dataset的数据读取 X Y的获取 Y包含两个 一个是附加损失的target 一个是target
